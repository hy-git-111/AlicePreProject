...
class LoginPage:
    ...
    random_wait = random.randrange(30,61)
    ...
    def input_user_data(self):
        ...
        email_input_box = self.driver.find_element(By.ID, self.EMAIL_ID)
        email_input_box.click()
        clipboard.paste()
        time.sleep(self.random_wait)

        clipboard.copy(user_data.PASSWORD)
        password_input_box = self.driver.find_element(By.ID, self.PASSWORD_ID)
        password_input_box.click()
        clipboard.paste()
        time.sleep(3)

        login_btn = self.driver.find_element(By.CLASS_NAME, self.LOGIN_BTN_CLASS_NAME)
